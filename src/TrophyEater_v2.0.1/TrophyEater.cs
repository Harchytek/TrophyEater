...
namespace TrophyEater
{
    ...
    public class TrophyEaterPlugin : BaseUnityPlugin
    {
        ...
        {
            {"TrophyEikthyr", "GP_Eikthyr"}, {"TrophyTheElder", "GP_TheElder"}, {"TrophyBonemass", "GP_Bonemass"},
            {"TrophyDragonQueen", "GP_Moder"}, {"TrophyGoblinKing", "GP_Yagluth"}, {"TrophySeekerQueen", "GP_Queen"}, {"TrophyFader", "GP_Fader"},
            {"TrophyDeer", "GP_Eikthyr"}, {"AncientSeed", "GP_TheElder"}, {"WitheredBone", "GP_Bonemass"},
            {"DragonEgg", "GP_Moder"}, {"GoblinTotem", "GP_Yagluth"}, {"DvergrKeyFragment", "GP_Queen"}, {"BellFragment", "GP_Fader"}
        };

        public static Dictionary<string, string> PowerTranslations = new Dictionary<string, string>
        {
            {"GP_Eikthyr", "Eikthyr"}, {"GP_TheElder", "The Elder"}, {"GP_Bonemass", "Bonemass"},
            {"GP_Moder", "Moder"}, {"GP_Yagluth", "Yagluth"}, {"GP_Queen", "The Queen"}, {"GP_Fader", "Fader"}
        };
        ...
        private void SetupTrophies()
        {
            ...
            var defaultXp = new Dictionary<string, float>
            {
                ...
                {"TrophyAbomination", 100f}, {"TrophyBlob", 20f}, {"TrophyDraugr", 20f}, {"TrophyDraugrElite", 30f}, {"TrophyLeech", 25f}, {"TrophySurtling", 25f}, {"TrophyWraith", 30f}, {"TrophyKvastur", 60f}, {"TrophyBonemass", 300f},
        ...
        {
            bool isFrench = Localization.instance.GetSelectedLanguage() == "French";
            string formatString = "";

            if (isFrench)
            {
                if (key == "TooltipPoints") formatString = "\nAjoute <color=orange>{0}</color> points pour <color=orange>{1}</color> skill.";
                else if (key == "TooltipPower") formatString = "\nAccorde le buff <color=orange>{0}</color> pour <color=orange>{1}</color> sec.";
                else if (key == "MessageSkill") formatString = "Vous augmentez {0} points de compétence pour {1}";
            }
            else
            {
                ...
                else if (key == "MessageSkill") formatString = "You increase {0} skill points to {1}";
            }
    ...
    public static class UseTrophyPatch
    {
        static bool Prefix(Humanoid __instance, Inventory inventory, ItemDrop.ItemData item)
        {
            Player player = __instance as Player;
            
            if (player == null || item?.m_dropPrefab == null || !TrophyEaterPlugin.TrophyConfigs.ContainsKey(item.m_dropPrefab.name)) 
                return true; 

            string prefabName = item.m_dropPrefab.name;

            if (TrophyEaterPlugin.BossItems.Contains(prefabName) && !TrophyEaterPlugin.EnableBossCategory.Value) return true;
            if (TrophyEaterPlugin.BossSummoningItems.Contains(prefabName) && !TrophyEaterPlugin.EnableBossSummoningCategory.Value) return true;

            var cfg = TrophyEaterPlugin.TrophyConfigs[prefabName];
            bool effetApplique = false;

            if (cfg.Points.Value > 0)
            {
                var type = TrophyEaterPlugin.GetSkillType(cfg.Skill.Value);
                player.RaiseSkill(type, cfg.Points.Value);
                player.Message(MessageHud.MessageType.Center, TrophyEaterPlugin.GetTranslation("MessageSkill", cfg.Points.Value, type));
                effetApplique = true;
            }

            if (cfg.Duration != null && cfg.Duration.Value > 0 && TrophyEaterPlugin.ItemToPower.TryGetValue(prefabName, out string seName))
            {
                ...
                if (effect != null)
                {
                    ...
                    player.GetSEMan().AddStatusEffect(cloned, true);
                    effetApplique = true;
                }
            }
            ...
            if (effetApplique)
            {
                var zanim = Traverse.Create(player).Field("m_zanim").GetValue<ZSyncAnimation>();
                if (zanim != null)
                {
                    zanim.SetTrigger("consume");
                }
    ...
    public static class TooltipPatch
    {
        ...
        {
            var type = typeof(ItemDrop.ItemData);
            var methods = type.GetMethods(BindingFlags.Public | BindingFlags.Instance)
                .Where(m => m.Name == "GetTooltip");
        ...
        static void Postfix(ItemDrop.ItemData __instance, ref string __result)
        {
            ...
            if (cfg.Points.Value > 0)
            {
                __result += TrophyEaterPlugin.GetTranslation("TooltipPoints", cfg.Points.Value, cfg.Skill.Value);
            }
            ...
            if (cfg.Duration != null && cfg.Duration.Value > 0)
            {
                if (TrophyEaterPlugin.ItemToPower.TryGetValue(prefabName, out string seName))
                {
                    ...
                    __result += TrophyEaterPlugin.GetTranslation("TooltipPower", powerName, cfg.Duration.Value);
                }
            }
        }
    }
